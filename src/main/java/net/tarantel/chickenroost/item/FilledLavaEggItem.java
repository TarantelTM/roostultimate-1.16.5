...
public class FilledLavaEggItem extends AbstractBucketItem {

    private final FlowingFluid fluids;


    public FilledLavaEggItem(Properties builder, FlowingFluid fluids) {
        super(Fluids.LAVA.delegate, builder);
        this.fluids = fluids;
    }
    ...
    public ITextComponent getName(@Nonnull ItemStack stack) {
        if (getFluid(stack) == Fluids.EMPTY) {
            return new TranslationTextComponent("item.chicken_roost.lava_egg");
        } else {
            ITextComponent fluidText = new TranslationTextComponent(getFluid(stack).getAttributes().getTranslationKey());
            return new TranslationTextComponent("item.chicken_roost.lava_egg.filled", fluidText);
        }
    }
