...
public class Trainer_Container extends Container {
    ...
    public Trainer_Container(int windowId, World world, BlockPos pos,
                               PlayerInventory playerInventory, PlayerEntity player) {


        super(ModContainers.TRAINER_CONTAINER.get(), windowId);
        ...
        if (tileEntity instanceof Trainer_Tile) {
            this.data = ((Trainer_Tile) tileEntity).getDataAccess();
        } else {
            this.data = new IntArray(2); // Fallback to a default IIntArray
        }
        ...
        if(tileEntity != null) {
            tileEntity.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY).ifPresent(handler -> {
                ...
                this.addSlot(new SlotItemHandler(handler, 1, 29, 38){
                    @Override
                    public boolean mayPlace(ItemStack stack) {
                        return ItemTags.getAllTags().getTagOrEmpty(new ResourceLocation("c:seeds/tiered")).contains(stack.getItem()) || ItemTags.getAllTags().getTagOrEmpty(new ResourceLocation("forge:seeds")).contains(stack.getItem());
                    }
                });
            });
        }
    ...
    public boolean stillValid(PlayerEntity playerIn) {
        return stillValid(IWorldPosCallable.create(tileEntity.getLevel(), tileEntity.getBlockPos()),
                playerIn, ModBlocks.TRAINER.get());
    }
