...
public class Trainer_Tile extends TileEntity implements ITickableTileEntity, IAnimatable {
    ...
    private static void craftItem(Trainer_Tile pEntity) {
        ...
        List<Trainer_Recipe> recipe = pEntity.getLevel().getRecipeManager()
                .getRecipesFor(ModRecipeTypes.TRAINER_RECIPE, inventory, pEntity.getLevel());
        if (level != null) {
    ...
    private <E extends TileEntity & IAnimatable> PlayState predicate(AnimationEvent<E> event) {
        switch (triggerAnim){
            ...
            case 1:
                event.getController().setAnimation(new AnimationBuilder().addAnimation("training", ILoopType.EDefaultLoopTypes.LOOP));
            default:
                event.getController().setAnimation(new AnimationBuilder().addAnimation("idle", ILoopType.EDefaultLoopTypes.LOOP));
        }
        //event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.gecko_habitat.idle", ILoopType.EDefaultLoopTypes.LOOP));
