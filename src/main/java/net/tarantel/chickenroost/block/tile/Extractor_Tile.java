...
import java.util.*;
public class Extractor_Tile extends TileEntity implements ITickableTileEntity {
    ...
    public Extractor_Tile(TileEntityType<?> tileEntityTypeIn) {
        super(tileEntityTypeIn);
    }

    public Extractor_Tile() {
        this(ModTileEntities.EXTRACTOR_TILE.get());
    }
    ...
    public CompoundNBT save(CompoundNBT compound) {
        ...
        compound.putInt("extractor.progress", this.progress);
    ...
    protected final IIntArray dataAccess = new IIntArray() {
        public int get(int pIndex) {
            switch(pIndex) {
                case 0:
                    return Extractor_Tile.this.progress;
                case 1:
                    return Extractor_Tile.this.maxProgress;
                default:
                    return 0;
            }
        }
        ...
        public void set(int pIndex, int pValue) {
            switch(pIndex) {
                case 0:
                    Extractor_Tile.this.progress = pValue;
                    break;
                case 1:
                    Extractor_Tile.this.maxProgress = pValue;
                    break;
            }
    ...
    public void craft() {
        ...
        Optional<Soul_Extractor_Recipe> recipe = level.getRecipeManager()
                .getRecipeFor(ModRecipeTypes.EXTRACTOR_RECIPE, inv, level);
    ...
    private static void craftItem(Extractor_Tile pEntity) {
        ...
        List<Soul_Extractor_Recipe> recipe = pEntity.getLevel().getRecipeManager()
                .getRecipesFor(ModRecipeTypes.EXTRACTOR_RECIPE, inventory, pEntity.getLevel());
        if (level != null) {
    ...
    private static boolean hasRecipe(Extractor_Tile entity) {
        ...
        Optional<Soul_Extractor_Recipe> recipe = Optional.empty();
        if (level != null) {
            // Get the recipe manager and find a matching recipe
            RecipeManager recipeManager = level.getRecipeManager();
            recipe = recipeManager.getRecipeFor(ModRecipeTypes.EXTRACTOR_RECIPE, inventory, level);

            // If a recipe is found, set the max progress based on the recipe's time
            if (recipe.isPresent()) {
                entity.maxProgress = Config.extractor_speedtimer.get() * recipe.get().getTime();
            }
        }
