...
public class Soul_Extractor_Block extends Block {
    ...
    public ActionResultType use(BlockState state, World worldIn, BlockPos pos,
        ...
        if(!worldIn.isClientSide) {
            ...
            if(tileEntity instanceof Extractor_Tile) {
    ...
    private INamedContainerProvider createContainerProvider(World worldIn, BlockPos pos) {
        return new INamedContainerProvider() {
            ...
            public Container createMenu(int i, PlayerInventory playerInventory, PlayerEntity playerEntity) {
                return new Extractor_Container(i, worldIn, pos, playerInventory, playerEntity);
            }
        };
    }
    @Override
    public void setPlacedBy(World pLevel, BlockPos pPos, BlockState pState, @Nullable LivingEntity pPlacer, ItemStack pStack) {
        ...
        if (!(blockEntity instanceof Extractor_Tile)) {
            return; // Exit early if the block entity is not of the expected type
        }

        Extractor_Tile tile = (Extractor_Tile) blockEntity;
    ...
    public void onRemove(BlockState pState, World pLevel, BlockPos pPos, BlockState pNewState, boolean pIsMoving) {
        if (pState.getBlock() != pNewState.getBlock()) {
            TileEntity blockEntity = pLevel.getBlockEntity(pPos);
            if (blockEntity instanceof Extractor_Tile) {
                ((Extractor_Tile) blockEntity).drops();
            }
        }
